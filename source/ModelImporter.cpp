...
ModelImporter::GenerateAssetFileName(const std::string &baseName,
  ...
  std::filesystem::path assetsPathObj(assetsDir);
  ...
  try {
    ...
    for (size_t i = 0; i < model.textures.size(); i++) {
      ...
      if (tex.source >= 0 &&
        ...
        if (!image.image.empty()) {
          // On Windows, stbi_write_png takes const char* which is ANSI.
          // This is still a limitation of stb, but we won't crash here now.
          stbi_write_png(texPath.string().c_str(), image.width, image.height,
    ...
    for (size_t i = 0; i < model.meshes.size(); i++) {
      ...
      for (size_t j = 0; j < mesh.primitives.size(); j++) {
        ...
        meshFile.write(reinterpret_cast<const char *>(&hasNormals),
                       sizeof(bool));
        if (hasNormals) {
...
ImportResult ModelImporter::ImportOBJ(const std::string &filePath,
  ...
  try {
    ...
    // LoadObj 签名: (attrib*, shapes*, materials*, err*, filename,
